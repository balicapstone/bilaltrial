...
// to do 
public class RegressionPGNNewHomepageTests {
    ...
    public static void executeBefore(){
    	...
    	try{
    		...
    		user.customWait().until(ExpectedConditions.visibilityOfElementLocated(home.statesModule));
    	} catch(Exception e){
	...
	*/
    ...
    //	user.customWait().until(ExpectedConditions.visibilityOfElementLocated(By.id("Dinosaurs")));
    	...
    	home.header.searchForArticle("illinois");
    	
    	PGONewArticlePage article = home.header.clickResultForSearch("Illinois");
    	
    	user.customWait().until(ExpectedConditions.visibilityOfElementLocated(article.logo));
    	
    	assertTrue("Asserts that the student user is taken to the Illinois article in States", 
    			article.getDriver().getCurrentUrl().equals("https://"+user.getEnvironment()+".pebblego.com/modules/6/categories/6994/articles/6013"));
    ...
    public void testSearchScreenReaderElementAttributes(){
		...
		assertTrue("Asserts that the search field web element has the appropriate placeholder information", 
				search.getAttribute("placeholder").toString().equals("Search PebbleGo Next")); 
    ...
    public void testSearchResultsScreenReaderElementAttributes(){
    	...
    	home.sendKeysToElement("earth ", home.header.searchBar);
